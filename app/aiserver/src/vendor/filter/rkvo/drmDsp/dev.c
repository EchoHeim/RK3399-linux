...
int is_supported_format(struct sp_plane* plane, uint32_t format) {
  ...
  for (i = 0; i < plane->plane->count_formats; i++) {
    if (plane->plane->formats[i] == format) return 1;
  }
  return 0;
}
...
struct sp_dev* create_sp_dev(void) {
  ...
  dev->planes =
      (struct sp_plane*)calloc(dev->num_planes, sizeof(struct sp_plane));
  for (i = 0; i < dev->num_planes; i++) {
    ...
    for (j = 0; j < dev->num_crtcs; j++) {
      if (plane->plane->possible_crtcs & (1 << j)) dev->crtcs[j].num_planes++;
    }
